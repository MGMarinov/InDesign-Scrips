...
var main = function() {
    ...
    /**
     * Sortiert die globalen Ausgabedaten nach der Seitennummer.
     */
    function sortiereDaten() {
        if (ausgabeText.length <= 1) return; // Nichts zu sortieren
        var header = ausgabeText.shift();
        ausgabeText.sort(function(a, b) {
            var seiteA = parseInt(a[1], 10);
            var seiteB = parseInt(b[1], 10);
            if (isNaN(seiteA) && isNaN(seiteB)) return 0;
            if (isNaN(seiteA)) return 1;
            if (isNaN(seiteB)) return -1;
            return seiteA - seiteB;
        });
        ausgabeText.unshift(header);
        $.writeln("Daten nach Seite sortiert.");
    }
    ...
    // Daten nach Seite sortieren
    sortiereDaten();

    // 5. Speicherort wählen mit modernem Dialog
    var zeitstempel = generiereZeitstempel();
    var dateiEndung = exportFormat === "json" ? ".json" : ".csv";
    var vorgeschlagenerDateiname = "artikel_liste_" + zeitstempel + dateiEndung;

    // Standard-Speicherort (Dokumentenordner oder Desktop)
    var defaultFolder = (doc.saved && doc.fullName) ? doc.fullName.parent : Folder.desktop;
    var defaultFile = new File(defaultFolder + "/" + vorgeschlagenerDateiname);

    // Modernen "Speichern unter"-Dialog anzeigen
    var dialogPrompt = "Speichern Sie die " + exportFormat.toUpperCase() + "-Datei";
    var filter = exportFormat === "json" ? "JSON:*.json" : "CSV:*.csv";
    
    filePath = defaultFile.saveDlg(dialogPrompt, filter);
