 ...
 * Aenderungen v2.2.0:
 * - Implementiert eine Fallback-Logik für gesperrte InDesign-Dateien.
 * - Wenn eine Datei nicht geöffnet werden kann, weil sie bereits in Gebrauch ist,
 * erstellt das Skript eine temporäre Kopie in einem "Ablage"-Unterordner,
 * verarbeitet diese Kopie und löscht sie anschließend.
 * - Dieser Kopiervorgang wird nur bei spezifischen "Datei kann nicht geöffnet werden"-Fehlern ausgelöst.
 *
 * Hinweise:
...
var SKRIPT_VERSION = "v2.2.0"; // Version erhöht
var SKRIPT_DATUM = "2025-10-27"; // Datum aktualisiert
...
function DocumentService(dependencies) {
    ...
    this.oeffneDokument = function(datei, userInteraction) {
        ...
        try {
            ...
            // Wichtig: Wir werfen den Originalfehler weiter, damit die aufrufende Funktion ihn analysieren kann.
            throw fehler;
        }
    };
...
function ExportCoordinator(dependencies) {
    ...
    this._verarbeiteItem = function(item, settings) {
        // Zuerst die INDD-Datei ermitteln, wie bisher
        ...
        if (!inddDatei || !inddDatei.exists) {
            throw new Error("INDD-Datei nicht gefunden für: " + (item.pdfName || (item.inddDatei && item.inddDatei.name) || "unbekannt"));
        }

        var doc = null;
        try {
            // --- ERSTER VERSUCH: Originaldatei öffnen ---
            doc = this.documentService.oeffneDokument(inddDatei, false);
            return this.dataProcessors.verarbeiteDokument(doc, settings, item);

        } catch (e) {
            // --- FEHLERBEHANDLUNG: Prüfen, ob die Datei gesperrt ist ---
            // Wir prüfen auf typische Meldungen für gesperrte/nicht zu öffnende Dateien.
            ...
            if (isLockedError) {
                this.logger.logWarnung("Datei '" + inddDatei.name + "' ist gesperrt. Erstelle temporäre Kopie in 'Ablage'.");

                var tempDoc = null;
                var tempDatei = null;
                try {
                    // "Ablage"-Ordner definieren und ggf. erstellen
                    var parentFolder = inddDatei.parent;
                    ...
                    // Pfad für die temporäre Datei
                    tempDatei = new File(ablageFolder.fsName + "/" + inddDatei.name);

                    // Evtl. vorhandene alte Kopie bereinigen
                    if (tempDatei.exists) {
                        tempDatei.remove();
                    }

                    // Datei kopieren
                    if (!inddDatei.copy(tempDatei)) {
                    ...
                    // --- ZWEITER VERSUCH: Temporäre Kopie öffnen ---
                    ...
                    // --- AUFRÄUMEN: Temporäre Dateien schließen und löschen ---
                    if (tempDoc) {
                ...
                // Es ist ein anderer Fehler, also werfen wir ihn weiter, um ihn normal zu protokollieren.
                throw e;
            }
        } finally {
            // --- AUFRÄUMEN (ERSTER VERSUCH): Das Originaldokument schließen, falls es geöffnet wurde ---
            if (doc) {
