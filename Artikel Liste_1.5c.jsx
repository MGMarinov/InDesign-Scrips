...
var main = function() {
    ...
    // 5. Speicherort und Dateiname automatisch generieren
    ...
    // Ordner für Export auswählen (Standard: Dokumentordner)
    var standardOrdner = new Folder(doc.filePath);
    var zielOrdner = Folder.selectDialog("Bitte wähle das Export-Verzeichnis:", standardOrdner);

    if (!zielOrdner) {
        $.writeln("Ordnerauswahl abgebrochen.");
        alert("Export abgebrochen.", SKRIPT_NAME);
        return;
    }

    // Vollständigen Dateipfad erstellen
    filePath = new File(zielOrdner.fsName + "/" + standardDateiName);

    $.writeln("Export-Datei: " + filePath.fsName);
