...
var main = function() {
    ...
    var benutzerDateiname = prompt("Bitte geben Sie den Dateinamen ein:", vorgeschlagenerDateiname, SKRIPT_NAME + " - Dateiname");

    if (!benutzerDateiname) {
    ...
    var lowerCaseName = benutzerDateiname.toLowerCase();
    if (lowerCaseName.indexOf(dateiEndung, lowerCaseName.length - dateiEndung.length) === -1) {
        benutzerDateiname += dateiEndung;
    }

    // Speicherort wählen (ohne voreingestellten Dateinamen)
    var ordnerPfad = Folder.selectDialog("Wählen Sie den Speicherordner aus");

    if (ordnerPfad) {
        filePath = new File(ordnerPfad.fsName + "/" + benutzerDateiname);
