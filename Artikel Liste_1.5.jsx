...
var main = function() {
     ...
     * @returns {Object} Ergebnis mit { erfolg: boolean, anzahl: number }
     */
                ...
                for (var i = 0; i < page.allPageItems.length; i++) {
                    ...
                    try {
                        if (item.itemLayer && item.itemLayer.name === layerName) {
                            ...
                            if (item.locked === true) {
                                gesperrteObjekte++;
                                $.writeln("Gesperrtes Objekt gefunden auf Seite " + page.name + ": " + item.constructor.name);
                            }
                        }
                    } catch (e) {
     ...
     * @param {number} anzahl Anzahl der gesperrten Objekte
     */
    function zeigeGesperrteObjekteFehler(anzahl) {
        var nachricht = "⚠ Gesperrte Objekte gefunden!\n\n";
        nachricht += "Auf der gewählten Ebene befinden sich " + anzahl + " gesperrte(s) Objekt(e).\n\n";
    ...
    /**
     * Generiert einen Zeitstempel im Format YYYY_MM_DD_HH_MM.
    ...
    function generiereZeitstempel() {
        ...
        return jahr + "_" + monat + "_" + tag + "_" + stunde + "_" + minute;
    }
    ...
    if (!gesperrteObjekteErgebnis.erfolg) {
        zeigeGesperrteObjekteFehler(gesperrteObjekteErgebnis.anzahl);
    ...
    if (filePath) {
        ...
        // Prüfen, ob Datei bereits existiert (zusätzliche Sicherheit)
        if (filePath.exists) {
            var ueberschreiben = confirm(
                "Die Datei existiert bereits:\n" + filePath.fsName + "\n\nMöchtest Du sie überschreiben?",
                false,
                SKRIPT_NAME + " - Datei überschreiben"
            );

            if (!ueberschreiben) {
                $.writeln("Benutzer hat das Überschreiben abgelehnt.");
                alert("Export abgebrochen. Die Datei wurde nicht überschrieben.", SKRIPT_NAME);
                return;
            }
        }
        ...
        alert("Speichervorgang abgebrochen. Keine Datei wurde gespeichert.", SKRIPT_NAME);
